...
const PriceCard = ({ plan: { planicon, title, text, plantype, plancontent, buttonText } }) => {
  ...
  return (
   <>
      <div className='p-5 rounded-lg  z-20 bg-slate-50/5 ring-1 ring-slate-100 shadow-md shadow-slate-200 hover:scale-105 duration-300 transition-all cursor-pointer'>
        <div className='flex items-center justify-between'>
            <div className='flex items-center gap-5'>
                ...
                <div className='grid items-center'>
                    <h1 className='text-gray-100 font-semibold text-lg filter drop-shadow-lg lg:text-base md:text-sm'>{title}</h1>
                    <p className='text-slate-900 text-white  font-normal text-sm lg:text-xs'>{text}</p>
                </div>
            </div>
        ...
        <div className='grid items-center gap-3 px-5'>
            {plancontent?.map((val, i) => (
                <div key={i} className="flex items-center justify-start gap-5">
                    ...
                    <div className='grid items-center'>
                        <h1 className='font-normal text-lg text-gray-100 filter drop-shadow-lg lg:text-base hover:text-emerald-500'>{val.text}</h1>
                    </div>
                </div>
            ))}
        </div>
        <div className='grid items-center justify-items-center mt-7'>
            <button type='button' className='   text-base'>{buttonText}</button>
        </div>
      </div>
   </>
  )
